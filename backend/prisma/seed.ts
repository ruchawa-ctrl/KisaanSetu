...
async function main() {
  ...
  const farmer = await prisma.user.create({ data: { name: 'Suresh Patil', phone: '9000000001', role: Role.FARMER, language: Language.MARATHI, verificationStatus: VerificationStatus.VERIFIED, rating: 4.7 } });
  ...
  await prisma.user.create({ data: { name: 'FreshCart Institutional Buyer', phone: '9000000003', role: Role.BUYER, language: Language.ENGLISH, verificationStatus: VerificationStatus.VERIFIED, rating: 4.6 } });
  ...
  const mandis = [{ name: 'Nashik', crop: 'Onion', base: 2450 }, { name: 'Pune', crop: 'Onion', base: 2310 }, { name: 'Latur', crop: 'Soybean', base: 4720 }];
